...
class WebScraperModule:
    ...
    def find_company_website(self, company_name):
        ...
        try:
            ...
            if potential_urls:
                ...
                potential_urls.sort(
                    key=lambda u: sum(
                        t in urlparse(u).netloc for t in self._tokenize_company_name(company_name.lower()) if len(t) > 2
                    ),
                    reverse=True,
                )
    ...
    def _is_potential_corporate_domain(self, url, company_name):
        ...
        try:
            ...
            non_corporate_domains = [
                ...
                "wsj.com",
                "forbes.com",
                "sec.gov",  # Aggiunto sec.gov per distinguerlo dai siti aziendali
                "gov.uk",
                "europa.eu",
                "duckduckgo.com",
                "yahoo.com",  # Aggiunti altri noti
                "pinterest.",
                "reddit.",
                "telegram.",
                "whatsapp.",
                "tiktok.",  # Social/Messaging
                "github.com",
                "gitlab.com",  # Codice sorgente
                "maps.",
                "microsoft.com",  # Domini troppo generici a volte
            ]
            ...
            significant_tokens = [
                ...
                if len(t) > 2 and t not in ["inc", "ltd", "the", "and", "corp", "plc", "ag", "sa", "nv", "bv"]
            ]
            ...
            return any(token in domain_name_part for token in significant_tokens) or any(
                len(token) > 4 and token.startswith(domain_name_part[:4]) for token in significant_tokens
            )
    ...
    def _tokenize_company_name(self, name):
        ...
        cleaned = re.sub(
            r"\b(inc|corp|corporation|ltd|limited|llc|group|holding|holdings|plc|ag|sas|spa|gmbh|co|company|incorporated|the|and|of|de|el|la)\b",
            "",
            cleaned,
            flags=re.IGNORECASE,
        )
    ...
    def find_investor_relations_page(self, company_url):
        ...
        try:
            ...
            for link in soup.find_all("a", href=True):
                ...
                is_ir_link = any(keyword in text for keyword in ir_keywords_text) or any(
                    keyword in href for keyword in ir_keywords_href
                )
                ...
                if is_ir_link or is_financial_link:
                    ...
                    if (
                        full_url and urlparse(full_url).netloc == urlparse(company_url).netloc
                    ):  # Assicurati sia sullo stesso dominio
                        score = 0
                        if is_ir_link:
                            score += 5
                        if "investor" in full_url:
                            score += 2
                        if "relations" in full_url:
                            score += 1
                        if is_financial_link and not is_ir_link:
                            score += 1  # Punteggio minore per link solo finanziari

                        # Penalizza link troppo generici o non specifici
                        if full_url.endswith((".pdf", ".zip", ".xlsx", ".docx")):
                            score -= 2  # Non è la pagina IR principale
                        if full_url == company_url:
                            score = -1  # Ignora se è la homepage stessa
    ...
    def find_financial_reports(self, page_url, source_type="Annual Report"):
        ...
        try:
            ...
            file_extensions = (
                ...
                ".htm",
                ".aspx",
            )  # Includi HTML/ASPX per report interattivi/pagine dedicate
            year_pattern = (
                r"(FY|Fiscal Year|Financial Year|)\s*(?<!\d)(20[1-3]\d)(?!\d|[-\/]\d)"  # Pattern Anno (2010-2039)
            )
            ...
            if "annual" in st_lower or "10-k" in st_lower or "consolidated" in st_lower:
                ...
                if "consolidated" in st_lower:
                    keywords = keywords[5:9] + keywords  # Priorità a consolidated
            ...
            for link in soup.find_all("a", href=True):
                ...
                if (
                    ...
                    or href_lower.endswith((".png", ".jpg", ".gif", ".svg"))
                ):
                    continue
                ...
                is_document_link = (
                    href_lower.endswith(file_extensions) or "download" in href_lower or "attachment" in href_lower
                )
                ...
                if has_keyword and is_document_link:
                    ...
                    if not year:
                        continue  # Ignora se non troviamo un anno valido
                    ...
                    if ".pdf" in href_lower:
                        score += 5  # PDF è spesso il report principale
                    if any(kw in link_text_lower for kw in keywords):
                        score += 3  # Keyword nel testo è più affidabile
                    if year in text:
                        score += 2  # Anno nel testo
                    if "annual report" in link_text_lower and "annual" in st_lower:
                        score += 2  # Match esatto tipo
                    if "10-k" in href_lower and "annual" in st_lower:
                        score += 3
                    if "10-q" in href_lower and "quarterly" in st_lower:
                        score += 3
                    if ".htm" in href_lower or ".aspx" in href_lower:
                        score += 1  # Pagine web dedicate sono ok
                    if ".zip" in href_lower:
                        score -= 1  # Meno preferibile
                    if "interactive" in href_lower:
                        score += 1  # Report interattivi
            ...
            relevant_years = [
                y for y in sorted_years if int(y) <= current_year and int(y) >= current_year - 5
            ]  # Considera ultimi 5 anni + anno corrente
    ...
    def _extract_year(self, text):
        ...
        year_range_pattern = (
            r"(20[1-3]\d)\s*[-\/]\s*(?:20|)(\d{2})\b"  # Match '2023-2024', '2023/2024', '2023-24', '2023/24'
        )
    ...
    def scrape_financial_sources(self, company_name, source_type):
        ...
        if company_url:
            ...
            if search_target_url:
                ...
                if reports:
                    ...
                    source_description = (
                        f"{source_type} from Company Website ({'IR Page' if ir_page_url else 'Homepage'})"
                    )
                    ...
                    logger.info(
                        f"Report trovato sul sito aziendale per '{company_name}': URL={best_url}, Anno={best_year}, Conf={confidence}"
                    )
            ...
            if sec_results:
                ...
                if best_url and best_year:  # Aggiunto controllo che best_year non sia None
                    ...
                    if int(sec_year) > int(best_year) or (int(sec_year) == int(best_year) and confidence != "ALTA"):
                        logger.info(
                            f"Filing SEC ({sec_form_type}, Anno {sec_year}) trovato più recente/affidabile del report sul sito. Aggiorno."
                        )
                        ...
                        logger.info(
                            f"Report dal sito web (Anno {best_year}) è preferito o uguale al filing SEC (Anno {sec_year}). Mantengo risultato sito web."
                        )
                else:
        ...
        if best_url and best_year:
            logger.info(
                f"Scraping completato per '{company_name}'. Risultato: URL={best_url}, Anno={best_year}, Fonte='{source_description}', Conf={confidence}"
            )
        else:
    ...
    def _could_be_us_company(self, company_name):
        ...
        us_indicators = [
            ...
            " Co.",
            " Corporation",
        ]  # Spazio prima per evitare match parziali
...
class PromptGenerator:
    ...
    def optimize_prompt(self, company_name, feedback, current_prompt, scraping_results=None):
        ...
        if optimization_attempt > max_optimizations:
            logger.warning(
                f"Raggiunto limite di {max_optimizations} ottimizzazioni per {company_name}. Mantengo ultimo prompt o uso fallback."
            )
        ...
        optimization_request = self._create_optimization_request_prompt(
        ...
        try:
            ...
            if len(optimized_prompt) > 200 and "{company_name}" in optimized_prompt and '"url":' in optimized_prompt:
                ...
                logger.warning(
                    f"Ottimizzazione prompt per {company_name} ha prodotto un risultato non valido o troppo corto. Mantengo prompt precedente. Risposta: {optimized_prompt[:200]}..."
                )
    ...
    def _generate_scraping_based_prompt(self, company_name, scraping_results, previous_prompt):
        ...
        if scraping_results and scraping_results[0] and scraping_results[1]:
            ...
            cleaned_base_prompt = re.sub(
                r"HINT SPECIFICO PER QUESTA AZIENDA.*?\n\n", "", self.base_prompt_template, flags=re.DOTALL
            )

            new_prompt = cleaned_base_prompt.format(
                company_name=company_name, source_type=source_type, optimization_instructions=scraping_suggestion
            )
            ...
            logger.warning(
                f"Scraping preliminare non ha fornito dati utili per {company_name}. Nessuna modifica al prompt precedente."
            )
    ...
    def validate_result(self, company_name, source_type_requested, url_found, year_found, scraping_results=None):
        ...
        if not url_found or not year_found:
            ...
            return {
                "validated": False,
                "feedback": {
                    "problems": ["URL o anno mancanti nella risposta del ricercatore."],
                    "suggestions": ["Il ricercatore deve fornire sia URL che anno."],
                },
            }
        try:
            ...
            int(year_found)
            if len(year_found) != 4:
                raise ValueError("Formato anno non YYYY")
        except (ValueError, TypeError):
            logger.warning(f"Validazione fallita (Anno non valido) per {company_name}: Anno='{year_found}'")
            return {
                "validated": False,
                "feedback": {
                    "problems": [f"Anno '{year_found}' non è in formato numerico YYYY."],
                    "suggestions": ["Il ricercatore deve fornire l'anno in formato YYYY."],
                },
            }
        ...
        if not url_accessible:
            ...
            return {
                "validated": False,
                "feedback": {
                    "problems": [f"L'URL fornito '{url_found}' non è accessibile o restituisce un errore."],
                    "suggestions": ["Verificare la correttezza dell'URL.", "Cercare un URL alternativo funzionante."],
                },
            }

        # Crea il prompt per il giudice Gemini
        validation_prompt = self._create_validation_prompt(
            company_name, source_type_requested, url_found, year_found, scraping_results
        )
        ...
        try:
            ...
            if validation_json:
                ...
                if "feedback" not in validation_json or not isinstance(validation_json["feedback"], dict):
                    validation_json["feedback"] = {
                        "problems": ["Feedback mancante o malformato dal validatore."],
                        "suggestions": [],
                    }
                ...
                logger.error(
                    f"Validazione fallita (Errore Interno Validatore) per {company_name}: Impossibile parsare la risposta del giudice."
                )
                return {
                    "validated": False,
                    "feedback": {
                        "problems": ["Errore interno del sistema di validazione (risposta non parsabile)."],
                        "suggestions": ["Riprovare."],
                    },
                }
        ...
        try:
            ...
            if json_match:
                ...
                if (
                    ...
                    and "validated" in data
                    and "feedback" in data
                    and isinstance(data["feedback"], dict)
                ):
                    # Assicura che problems e suggestions siano liste
                    data["feedback"]["problems"] = data["feedback"].get("problems", [])
                    if not isinstance(data["feedback"]["problems"], list):
                        data["feedback"]["problems"] = [str(data["feedback"]["problems"])]

                    data["feedback"]["suggestions"] = data["feedback"].get("suggestions", [])
                    if not isinstance(data["feedback"]["suggestions"], list):
                        data["feedback"]["suggestions"] = [str(data["feedback"]["suggestions"])]
                ...
                logger.warning(
                    f"Nessun JSON valido trovato nella risposta di validazione per {company_name}. Risposta: {response_text[:200]}..."
                )
                return None
...
class FinancialSourceFinder:
    ...
    def __init__(
        self, csv_path, api_key, output_csv="financial_sources_output.csv", max_workers=5, max_iterations=3
    ):  # Ridotto max_iterations default
        """
        ...
        if not api_key:
            logger.critical(
                "API Key di Google Gemini non fornita. Impostala come variabile d'ambiente GOOGLE_API_KEY o usa l'argomento --api_key."
            )
            sys.exit(1)
        ...
        try:
            ...
            if not any("generateContent" in m.supported_generation_methods for m in genai.list_models()):
                ...
                sys.exit(1)
            logger.info(
                f"API Key Google Gemini configurata correttamente. Modelli disponibili (estratto): {[m for m in models if 'gemini' in m][:5]}"
            )
        except Exception as e:
    ...
    def load_companies(self):
        ...
        try:
            ...
            if not all(col in df.columns for col in required_cols):
                logger.critical(
                    f"Il file CSV deve contenere le colonne 'CompanyName' e 'SourceType' (ignora maiuscole/minuscole). Colonne trovate: {list(df.columns)}"
                )
                sys.exit(1)
            ...
            if missing_source_type.any():
                logger.warning(
                    f"{missing_source_type.sum()} righe con SourceType mancante. Verrà usato il default: '{default_source_type}'"
                )
        ...
        try:
            ...
            if json_match:
                ...
                if not isinstance(data, dict) or not required_keys.issubset(data.keys()):
                    logger.warning(
                        f"JSON ricerca incompleto ricevuto per {company_name}. Chiavi mancanti o struttura errata. JSON: {data}"
                    )
                    return None
                ...
                if not (data["url"] is None or isinstance(data["url"], str)):
                    logger.warning(
                        f"Tipo non valido per 'url' in JSON ricerca per {company_name}: {type(data['url'])}. JSON: {data}"
                    )
                    return None
                if not (data["year"] is None or isinstance(data["year"], str)):  # Year è atteso come stringa YYYY
                    ...
                    if isinstance(data["year"], int) and len(str(data["year"])) == 4:
                        ...
                        logger.warning(
                            f"Tipo o formato non valido per 'year' in JSON ricerca per {company_name}: {data['year']}. JSON: {data}"
                        )
                        return None
                if not isinstance(data["confidence"], str) or data["confidence"].upper() not in [
                    "ALTA",
                    "MEDIA",
                    "BASSA",
                ]:
                    logger.warning(
                        f"Valore non valido per 'confidence' in JSON ricerca per {company_name}: {data['confidence']}. JSON: {data}"
                    )
                    # Forziamo un default se non valido? O scartiamo? Per ora scartiamo.
                    return None
                if not isinstance(data["source_type_found"], str):
                    logger.warning(
                        f"Tipo non valido per 'source_type_found' in JSON ricerca per {company_name}: {type(data['source_type_found'])}. JSON: {data}"
                    )
                    return None
                ...
                logger.warning(
                    f"Nessun JSON valido trovato nella risposta di ricerca per {company_name}. Risposta: {response_text[:200]}..."
                )
                return None
        except json.JSONDecodeError as e:
            ...
            logger.error(
                f"Errore generico durante il parsing della risposta di ricerca per {company_name}: {e}", exc_info=True
            )
            return None
        ...
        try:
            ...
            if not response.parts:
                ...
                if response.prompt_feedback and response.prompt_feedback.block_reason:
                    logger.error(
                        f"Prompt bloccato per {company_name}. Ragione: {response.prompt_feedback.block_reason}"
                    )
    ...
    def process_company(self, company_data):
        ...
        try:
            ...
            logger.info(
                f"{log_prefix} Risultati scraping preliminare: {scraping_results[:2]}"
            )  # Logga solo URL e Anno per brevità
        except Exception as e:
        ...
        for iteration in range(self.max_iterations):
            ...
            try:
                ...
                logger.error(
                    f"{log_prefix} Errore chiamata/parsing Gemini Search (Iterazione {actual_iterations}): {e}"
                )
                ...
                final_feedback = {
                    "problems": [f"Errore API Gemini Search: {e}"],
            ...
            if not gemini_result or gemini_result.get("url") is None:
                ...
                final_feedback = {
                    "problems": ["Ricerca Gemini non ha prodotto un URL valido o risultato parsabile."],
                    "suggestions": ["Riformulare prompt.", "Verificare nome azienda e tipo fonte."],
                }
                ...
                if iteration < self.max_iterations - 1:
                    ...
                    current_prompt = self.prompt_generator.optimize_prompt(
                        company_name, final_feedback, current_prompt, scraping_results
                    )
            ...
            try:
                ...
                final_feedback = validation_response.get(
                    "feedback", {}
                )  # Aggiorna il feedback con quello del validatore
                ...
                if validated:
                    ...
                    if iteration < self.max_iterations - 1:
                        ...
                        logger.warning(
                            f"{log_prefix} Massimo numero di iterazioni ({self.max_iterations}) raggiunto senza validazione."
                        )
                ...
                logger.error(
                    f"{log_prefix} Errore grave durante la validazione (Iterazione {actual_iterations}): {e}",
                    exc_info=True,
                )
    ...
    def save_results(self):
        ...
        try:
            ...
            logger.error(
                f"Errore durante la creazione o il salvataggio del DataFrame dei risultati in CSV: {e}", exc_info=True
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Trova fonti finanziarie per MNE usando Gemini, Web Scraping e Auto-Tuning dei Prompt."
    )
    parser.add_argument(
        "csv_path",
        help="Percorso del file CSV di input (colonne obbligatorie: CompanyName, SourceType - ignora maiuscole/minuscole)",
    )
    parser.add_argument(
        "-k",
        "--api_key",
        default=os.environ.get("GOOGLE_API_KEY"),
        help="API Key di Google Gemini (predefinita: legge variabile d'ambiente GOOGLE_API_KEY)",
    )
    parser.add_argument(
        "-o",
        "--output",
        default="financial_sources_output.csv",
        help="Nome del file CSV di output (predefinito: financial_sources_output.csv)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Numero massimo di cicli Ricerca->Validazione->Ottimizzazione per azienda (predefinito: 3)",
    )
    ...
    logger.info("Log dettagliato disponibile in: financial_sources_finder.log")
